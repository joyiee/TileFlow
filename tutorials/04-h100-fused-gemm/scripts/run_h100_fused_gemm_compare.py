...
DEFAULT_ACTUAL_CSV = Path(
    ...
    "H100/Fused GEMM/Validation Data/mnk_max_block_span_512_2048_fused_silu.csv"
)
DEFAULT_OUT_CSV = TUTORIAL_DIR / "results" / "mnk_max_block_span_512_2048_fused_silu_compare.csv"
...
def read_actual(csv_path):
    ...
    with open(csv_path, newline="") as f:
        for row in csv.DictReader(f):
            rows.append((int(row["M"]), int(row["N"]), int(row["K"]), int(float(row["max_block_span_cycles"]))))
    return rows
...
def main():
    ...
    for idx, (m, n, k, actual_cycles) in enumerate(actual_rows, start=1):
        ...
        results.append((m, n, k, "fused_silu", actual_cycles, sim_cycles))

    with open(out_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["M", "N", "K", "tag", "actual_cycles", "sim_cycles"])
