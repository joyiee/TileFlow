...
DEFAULT_ACTUAL_CSV = Path(
    ...
    "A100/GEMM/Validation Data/Multiple_SM_BMBNBK12825664_PERSISTENT/"
    "mnk_max_sm_span_512_2048_2stage.csv"
)
DEFAULT_OUT_CSV = TUTORIAL_DIR / "results" / "mnk_max_sm_span_512_2048_2stage_compare.csv"
...
def read_actual(csv_path):
    ...
    with open(csv_path, newline="") as f:
        for row in csv.DictReader(f):
            rows.append((int(row["M"]), int(row["N"]), int(row["K"]), int(float(row["max_sm_span"]))))
    return rows
