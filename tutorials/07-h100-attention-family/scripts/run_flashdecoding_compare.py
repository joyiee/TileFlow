...
DEFAULT_OUT_CSV = TUTORIAL_DIR / "results" / "flashdecoding_overall_max_span_compare_tileflow.csv"
HEAD_DIM = 128
...
def main():
    ...
    raw_dir = out_path.parent / "flashdecoding_raw"
    macro_dir = out_path.parent / "flashdecoding_macro"
    ...
    for idx, (batch, heads_q, heads_kv, seq_q, seq_kv, actual_cycles) in enumerate(actual_rows, start=1):
        ...
        qk_cycles = run_gemm_case(args.tileflow, raw_dir, macro_dir, f"flashdecoding_{idx:02d}_qk", m, seq_kv, HEAD_DIM)
        pv_cycles = run_gemm_case(args.tileflow, raw_dir, macro_dir, f"flashdecoding_{idx:02d}_pv", m, HEAD_DIM, seq_kv)
